...
const Card = () => {
  ...
  if (error) return <p>Error loading gyms</p>;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 xl:gap-y-6 p-6 lg:px-12 xl:px-20 mb-14">
      {data?.data?.map((item) => (
        <div
          ...
          className="bg-white rounded-2xl shadow-lg p-6 max-w-[380px] mx-auto cursor-pointer"
        >
          ...
          <div className="flex items-center gap-2">
            <img
              ...
              className="h-[35px]"
            />
            <p className="text-gray-500 text-sm sm:text-base">{item.gymLocation?.name}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-4">
            {item.facilities?.map((f, i) => (
              <div key={i} className="flex flex-col items-center justify-center rounded-lg p-2">
                {f.icons?.length > 0 && (
                  <img
                    src={`http://localhost:5000/uploads/${f.icons[0]}`}
                    alt={f.name}
          ...
          <div className="flex gap-3 mt-2.5 px-2">
            {item.openingIcon && (
             <img src={`http://localhost:5000/uploads/${item.openingIcon}`} alt="icon" className="w-10 h-10 object-contain" />
            )}
            <div>
